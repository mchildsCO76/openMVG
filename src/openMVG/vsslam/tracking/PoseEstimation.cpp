...
namespace VSSLAM  {
  ...
  double PoseEstimator::computeHomographyScore
  (
      const Mat & model,
      const Mat & x1,
      const Mat & x2,
  ...
  double PoseEstimator::computeEpipolarScore
(
    const Mat & model,
    const Mat & x1,
    const Mat & x2,
...
)
{
  ...
  bool PoseEstimator::estimate_Rt_from_E
  (
    const Mat3 & K1,
    const Mat3 & K2,
    const Mat & x1,
  ...
  bool PoseEstimator::estimateRobustRelativePosePinholeHE
  (
    const Frame * frame_1,
    const Frame * frame_2,
  ...
  )
  {
    ...
    std::cout<<"Pose: [Compute H] " << bValid_H << " thresh_sq: " << infoACR_H.first<<" NFA: "<<infoACR_H.second <<" inliers: "<<vec_inliers_H.size() << " ( "<<omp_get_wtime() - startTimeB<<" s)\n";

    startTimeB = omp_get_wtime();
    // ----------------------------
    // Estimate  E: cur_E_init
    // ----------------------------
    Mat3 E;
    ...
    std::cout<<"Pose: [Compute E] " << bValid_E << " thresh_sq: " << infoACR_E.first<<" NFA: "<<infoACR_E.second <<" inliers: "<<vec_inliers_E.size() << " ( "<<omp_get_wtime() - startTimeB<<" s)\n";
    ...
    else
    {
      ...
      Mat3 R2; Vec3 t2; double s2;

      // Estimate rotation and translation from essential matrix
      //if (estimate_Rt_from_E(cam_1_K, cam_1_K, pt2D_frame1, pt2D_frame2, E, vec_inliers_E, min_cos_angle_init_points, R, t))
      if (sfm::estimate_Rt_fromE(cam_1_K, cam_2_K, pt2D_frame1, pt2D_frame2, E, vec_inliers_E, &R2, &t2))
      {

        // Keep only inliers that have enough paralax
        Mat34 P1, P2;
        Mat R1 = Mat3::Identity();
        Vec3 t1 = Vec3::Zero();
        P_From_KRt(cam_1_K, R1 , t1, &P1);
        P_From_KRt(cam_2_K, R2, t2, &P2);

        // 3D point
        Vec3 X;

        for (std::vector<size_t>::iterator it_inlier = vec_inliers_E.begin(); it_inlier != vec_inliers_E.end();)
        {
          size_t k = *it_inlier;
          const Vec2
          ...
          const double dotAngle = ray1.dot( ray2 );
          const double cosParallax = clamp( dotAngle / mag, -1.0 + 1.e-8, 1.0 - 1.e-8 );
          // If cosParallax is too big (the angle is to small)
          if (cosParallax > min_cos_angle_init_points)
          {
            it_inlier = vec_inliers_E.erase(it_inlier);
          }
          else
        ...
        AC_threshold_sq = infoACR_E.first;
        s2 = 1.0;
        T.block(0,0,3,3) = s2 * R2;
        T.block(0,3,3,1) = t2;
        T(3,3) = 1;
        return true;
      }
  ...
  bool PoseEstimator::estimateRobustPose
  (
    const Frame * frame,
  ...
  )
  {
    ...
    // Try to estimate the H and F/E from mathces
    const size_t n_putative_matches = vec_putative_matches_3D_frame_idx.size();
    ...
    for (int m_i = 0; m_i < n_putative_matches; ++m_i)
    {
      Hash_Map<MapLandmark*,IndexT>::iterator m_iter = vec_putative_matches_3D_frame_idx.begin();
      std::advance(m_iter,m_i);
...
*/
  ...
  bool PoseEstimator::getRelativePointPosition(const Vec3 & pt, const Frame * ref_point_frame, Vec3 & pt_new, const Frame * new_point_frame )
  {
  ...
  */
  ...
  bool PoseEstimator::computeFundamentalMatrix(Frame * frame_1, Frame * frame_2, Mat3 & F)
  {
    Mat4 T_2_1;
    ...
    const Mat3 sR = T_2_1.block(0,0,3,3);
    ...
    F = K_inv_2.transpose() * tx * sR * K_inv_1;
    return true;
