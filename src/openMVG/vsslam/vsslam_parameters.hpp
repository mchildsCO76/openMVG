...
struct VSSLAM_Parameters : public std::enable_shared_from_this<VSSLAM_Parameters>
{
  ...
  float track_match_mm_win_size = 5;
  ...
  float triang_match_desc_ratio = 0.9; // Matching ratio for matching using motion model
  float triang_match_max_scale_ratio = 1.1; // Matching can be done only between points with max twice the scale difference
  float triang_match_max_epipolar_distance = 8.0; // Max distance to epipolar line
  // ---------------
  // Mapping parameters
  // ---------------
  ...
  size_t map_max_inactive_f_local_landmark = 5; // Maximum frames of inactivity that local landmark survives
