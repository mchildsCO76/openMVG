...
)
{
  ...
  for (auto & match : matches_3D_pts_frame_i_idx)
  {
    ...
    ceres::CostFunction* cost_function = IntrinsicsToCostFunction(cam_i_intrinsic, frame_i->getFeaturePosition(match.second));
  ...
  else // Solution is usable
  {
    ...
    for (auto & it_frame : tmp_frames)
    {
      ...
      if (frame->isActive())
        continue;
      // Get frame and update values
      IndexT frame_id = frame->getFrameId();
      ceres::AngleAxisToRotationMatrix(&(map_poses[frame_id][0]), R_refined.data());
...
)
{
  ...
  if (frame_i)
  {
    ...
    if(matches_3D_pts_frame_i_idx && !matches_3D_pts_frame_i_idx->empty())
    {
      // Add all 2D-3D relations of the free pose
      for (auto & match : *matches_3D_pts_frame_i_idx)
      {
