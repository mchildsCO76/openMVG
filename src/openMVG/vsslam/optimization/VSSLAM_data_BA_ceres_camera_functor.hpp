...
struct Chi2ErrorFunctor_Pinhole_Intrinsic_Rts
{
  ...
  :m_pos_2dpoint(pos_2dpoint), m_inf_2dpoint(inf_2dpoint)
  {
  }
    ...
    T* out_residuals) const
  {
    ...
    out_residuals[0] = T(m_inf_2dpoint[0]) * e_x + T(m_inf_2dpoint[2]) * e_y;
    out_residuals[1] = T(m_inf_2dpoint[1]) * e_x + T(m_inf_2dpoint[3]) * e_y;
      ...
      return
        (new ceres::AutoDiffCostFunction
          <Chi2ErrorFunctor_Pinhole_Intrinsic_Rts, 2, 3, 7, 3>(
            new Chi2ErrorFunctor_Pinhole_Intrinsic_Rts(observation.data(),inf_matrix.data())));
    }
    else
    {
      return
        (new ceres::AutoDiffCostFunction
          ...
          (new WeightedCostFunction<Chi2ErrorFunctor_Pinhole_Intrinsic_Rts>
            (new Chi2ErrorFunctor_Pinhole_Intrinsic_Rts(observation.data(),inf_matrix.data()), weight)));
    }
  }

  const double * m_pos_2dpoint; // The 2D observation
  const double * m_inf_2dpoint; // The information of 2D observation
};
